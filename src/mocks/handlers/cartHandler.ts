...
import { mockCartItemResponse } from '../dummys/cartItemDummy';
import { mockProductResponse } from '../dummys/productDummy';

let cartItems = [...mockCartItemResponse.content];

const findProductById = (id: number) => {
  const product = mockProductResponse.content.find((product) => product.id === id);
  return product;
};
...
export const cartHandlers = [
  ...
  http.post('https://example.com/cart-items', async ({ request }) => {
    ...
    if (!product) return errorResponse('PRODUCT_NOT_FOUND', '존재하지 않는 상품입니다.', 404);

    if ((product?.quantity ?? 10000) < quantity) {
      return errorResponse('OUT_OF_STOCK', '재고 수량을 초과하여 담을 수 없습니다.', 400);
    }
  ...
  http.put('https://example.com/cart-items/:id', async ({ params, request }) => {
    const id = Number(params.id);
    const { quantity } = (await request.json()) as { quantity: number };

    const item = cartItems.find((item) => item.id === id);

    if (!item) return errorResponse('CART_NOT_FOUND', '존재하지 않는 장바구니 상품입니다.', 404);
