...
export const handlers = [
  ...
  http.get(`${CONFIG.apiUrl}${PATH.products}`, async () => {
    if (maybeError()) {
  ...
  http.get(`${CONFIG.apiUrl}${PATH.cartItems}`, async () => {
    if (maybeError()) {
      await delay(MOCK_DELAY);
      return HttpResponse.json({ message: "랜덤 에러 발생!" }, { status: 500 });
    }
  ...
  http.delete(`${CONFIG.apiUrl}${PATH.cartItems}/:cartItemId`, async ({ params }) => {
    if (maybeError()) {
      await delay(MOCK_DELAY);
      return HttpResponse.json({ message: "랜덤 에러 발생!" }, { status: 500 });
    }
