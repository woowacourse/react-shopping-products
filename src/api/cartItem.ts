...
import fetchWithErrorCheck from '@/api/utils/fetchWithErrorCheck';

export const getCartList = async (): Promise<CartItem[]> => {
  const data = await fetchWithErrorCheck<{ content: CartItem[] }>(
...
export const getCartListQuantity = async (): Promise<number> => {
  const data = await fetchWithErrorCheck<{ quantity: number }>(
...
export const addCartItem = async (productId: number) => {
  await fetchWithErrorCheck<void>(
    () =>
