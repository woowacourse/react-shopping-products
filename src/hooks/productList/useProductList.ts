...
const useProductList = (filtering: Filtering) => {
  ...
  const queryFn :QueryFunction<FetchProductListReturnType, QueryKey, unknown>  =async ({ pageParam = 0 }) => fetchProductList({ filtering, page: pageParam as number });
  const {data,fetchNextPage ,error ,refetch ,status} = useInfiniteQuery<FetchProductListReturnType, Error>({
    ...
    getNextPageParam: (params : FetchProductListReturnType|
      undefined
    ) => {
