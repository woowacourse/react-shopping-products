...
import useCheckedId from '../src/hooks/product/useCheckedId';
...
describe('장바구니 버튼 토글 테스트', () => {
  it('"담기" 버튼을 누르면 총 장바구니 갯수는 1개 증가한다.', async () => {
    ...
    const initShoppingCartItemLength = result.current.length;

    await act(async () => {
      result.current.toggleId(TARGET_MOCK_DATA.id);
    });

    expect(result.current.length).toBe(initShoppingCartItemLength + 1);
  });
  ...
  it('"담기" 버튼을 누르고 "빼기" 버튼을 누르면 총 장바구니 갯수는 0개다.', async () => {
    ...
    expect(result.current.length).toBe(initShoppingCartItemLength);
  });
  ...
  it('네트워크 에러 상황에서 담기 버튼을 누르면 장바구니가 담기지 않아야 한다.', async () => {
    ...
    await act(async () => {
      try {
        await result.current.toggleId(TARGET_MOCK_DATA.id);
      } catch (error) {
